<template>
  <div>
    <div
      class="text-amber-50 flex flex-col space-y-4"
      v-if="!useHomeStore().loading"
    >
      <Card
      ...
      >
        <CardHeader>
          ...
          >
            {{ i.author }}
            -
            {{ new Date(i.publishedAt * 1000).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) }}
          </CardDescription>
        </CardHeader>
      </Card>
    </div>
    <LoadingComponent v-if="useHomeStore().loading" />
  </div>
</template>
